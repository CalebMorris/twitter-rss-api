    ...
    for(var mediaItem of mediaItems) {
      switch(mediaItem.type) {
        ...
        case 'video':
          ...
          break;
        case 'animated_gif':
          mediaBlock += buildMediaAnimatedGif(mediaItem);
          break;
        default:
...
function buildMediaAnimatedGif(mediaItem) {
  if (!mediaItem || !mediaItem.video_info || !mediaItem.video_info.variants) {
    console.warn(`[T-${mediaItem.id_str}] Missing required content to embed a video`);
    return '';
  }
  ...
  return `
    <div class="tweet-media tweet-animated-gif">
      <video autoplay="true" loop="true" muted="true" poster="${mediaItem.media_url_https}">${sources}
