...
async function handler(twit, request, h) {
  try {
    ...
    const timelineTweets = await twit.statuses.timeline({
    ...
    _.map(timelineTweets, (timelineTweet) => {
      if (timelineTweet.in_reply_to_status_id_str) {
        timelineTweet.in_reply_to_tweet = innerTweetsMap[timelineTweet.in_reply_to_status_id_str];
      }
